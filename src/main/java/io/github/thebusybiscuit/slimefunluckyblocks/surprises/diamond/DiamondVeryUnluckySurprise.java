...
public class DiamondVeryUnluckySurprise implements Surprise {
    @Override
    public String getName() {
        return "DiamondVeryUnluckySurprise";
    }

    @Override
    public LuckLevel getLuckLevel() {
        return LuckLevel.DIAMOND_VERY_UNLUCKY;
    }
    ...
    public void activate(Random random, Player p, Location l) {
        if (random.nextInt(100) < 60) {
            new DiamondVeryLuckySurprise().activate(random, p, l);
            return;
        }

        int numDrops = getNumberOfDrops(random);
        int entityChoice = random.nextInt(10); // 0-9
        ...
        for (int i = 0; i < numDrops; i++) {
            ...
            if (eventType == 0) { // Items
                ...
                pool.add(new ItemStack(Material.STRING, 2));
                ...
                } else if (entityChoice >= 3 && entityChoice <= 8) {
                    for(int j=0; j<2; j++) l.getWorld().spawnEntity(l, EntityType.PHANTOM);
                } else {
                    l.getWorld().spawnEntity(l, EntityType.WARDEN);
                }
            } else { // Potions
